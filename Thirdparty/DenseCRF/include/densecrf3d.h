...
public:
    ...
    explicit DenseCRF3D( int N ) : DenseCRF(N) {
    ...
    ~DenseCRF3D() override {
	delete[] unary_; unary_ = nullptr; 
	delete[] current_;current_ = nullptr;
	delete[] next_;next_ = nullptr;
	delete[] tmp_;tmp_ = nullptr;
        delete[] map_;map_ = nullptr;
    }

    DenseCRF3D( DenseCRF3D & o ) = delete;
    ...
    void SetUnaryEnergtForPositiveNode(int idx, int m, float value);
};
...
template<int M>
void DenseCRF3D<M>::expAndNormalize( float* out, const float* in, float scale /* = 1.0 */, float relax /* = 1.0 */ ) {
    auto *V = new float[ M ];
    for( int i=0; i<N_; i++ ){
...
template<int M>
void DenseCRF3D<M>::setUnaryEnergyFromLabel(const short* label, float confidence) {
    float confidences[M];
...
template<int M>
void DenseCRF3D<M>::setUnaryEnergyFromLabel(const short* label, float* confidences) {
...
template<int M>
void DenseCRF3D<M>::SetUnaryEnergtForPositiveNode(int idx, int m, float value){
   unary_[idx * M + m] = value;
}

template<int M>
void DenseCRF3D<M>::buildMap() {
